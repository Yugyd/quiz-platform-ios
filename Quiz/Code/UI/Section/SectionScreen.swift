...
struct SectionScreen: View {
    ...
    var onNavigateToGame: (SectionSenderArgs) -> Void
    var onBack: () -> Void
    ...
    var body: some View {
        ZStack {
            if viewModel.isLoading {
                ...
                SectionListContent(
                    sections: viewModel.items,
                    onSectionClicked: { section in
                        viewModel.onAction(action: .onSectionClicked(section))
                    }
                )
            }
        }
        .onChange(of: viewModel.navigationState) { navigationState in
            ...
            case .navigateToGame(let args):
                onNavigateToGame(args)
            case .back:
                onBack()
            case .none:
                break
