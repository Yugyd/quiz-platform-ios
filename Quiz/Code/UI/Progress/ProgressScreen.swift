...
struct ProgressScreen: View {
    ...
    var onNavigateToSpecificProgress: (ProgressUiModel) -> Void
    ...
    var body: some View {
        ZStack {
            if viewModel.isLoading {
                ...
                ProgressListView(
                    ...
                    onItemClicked: { progress in
                        viewModel.onAction(action: .onProgressClicked(item: progress))
                    }
                )
            }
        }
        .onChange(of: viewModel.navigationState) { navigationState in
            ...
            case .navigateToSpecificProgress(let progress):
                onNavigateToSpecificProgress(progress)
            case .none:
                break
            }
