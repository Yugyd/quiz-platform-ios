...
class ContinueGameProcessDelegate: GameProcessDelegate {

    let conditionValueMin = 0

    func initCondition() -> Int {
        return GameProcess.lifeCondition
    }
    ...
    func specDecrementCondition(gameData: Game) {
        fatalError("Arcade and marathon mode non supported spec decrement condition")
    }
