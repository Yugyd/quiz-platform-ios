...
@MainActor
class SubCourseListViewModel: ObservableObject {
    
    private let loggerTag = "SubCourseListViewModel"
    ...
    @Published var initialArgs: SubCoursesInitialArgs = SubCoursesInitialArgs.empty
    @Published var navigationState: SubCourseNavigationState?
    ...
    init(
        ...
        self.parentCourseTitle = initialArgs.courseTitle
    }
    
    func onAction(_ action: SubCourseListAction) {
        switch action {
        case .loadData:
            loadData()
    ...
    private func loadData() {
        Task { [weak self] in
            ...
            do {
                let courses = try await courseInteractor.getCourses(parentCourseId: initialArgs.courseId)
